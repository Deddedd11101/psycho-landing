...
export function confirmationForClient(session: SessionRecord): string {
  ...
  if (session.intent === 'contact' || !session.slot) {
    return [
      `Здравствуйте, ${name}! 👋`,
      '',
      'Ваша анкета получена — психолог уже видит ваш запрос и ответит в ближайшее время (обычно в течение нескольких часов).',
      '',
  ...
  return [
    ...
    place,
    '',
    `💬 <b>Контакт психолога:</b> ${psychologistLink}`,
    '',
    'Длительность встречи — 50 минут. Если планы изменятся, пожалуйста, предупредите не позднее чем за 24 часа.',
    '',
    '<i>До встречи!</i>',
  ].join('\n');
}
...
export function welcomeMessage(): string {
  return [
    '👋 <b>Здравствуйте!</b>',
    '',
    'Это бот записи к психологу. Через него приходят подтверждения записи и напоминания.',
    '',
